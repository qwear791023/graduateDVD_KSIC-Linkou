...
function jsOutput_HTML(dataFile) {
    ...
    if(document.body.id === 'photos') {
        ...
        if(dataFile == 'uniform_personal') musicDOM.src = '../audio/001.mp3';
        if(dataFile == 'casual_personal') musicDOM.src = '../audio/002.mp3';
        if(dataFile == 'uniform_team') musicDOM.src = '../audio/003.mp3';
        if(dataFile == 'sport_team') musicDOM.src = '../audio/004.mp3';
        if(dataFile == 'climb') musicDOM.src = '../audio/005.mp3';
        if(dataFile == 'graduate') musicDOM.src = '../audio/006.mp3';
    ...
    if(document.body.id === 'videos') {
        console.log('jsOutput_HTML >>> videos_Page');

        const video = document.querySelector('.list_content video');
        video.src = `../video/${dataFile}.mp4`;
    }
}
...
if (document.body.id === 'photos') {
    jsOutput_HTML('uniform_personal');
    initCardSliderEvents();
}


if(document.body.id === 'videos') {
    console.log('videos__Pages');
    
    const videoBtns = document.querySelectorAll('[data-file]');
    videoBtns.forEach(el => {
        el.addEventListener('click', function () {
            if(el.className.includes('active')) return;
            videoBtns.forEach(e => e.classList.remove('active'));
            el.classList.add('active');
            jsOutput_HTML(el.dataset['file']);
        });
    });
